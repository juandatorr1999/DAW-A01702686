...
function getcotizaciones(){
     $conexion = connect();
        ...
        $cotizaciones = '<div class = "row justify-content-center"> <div class = "col-5"> <table class = "table text-center table-hover"><tr><th>Accion</th><th>Precio</th></tr>';
        ...
        while ($row = mysqli_fetch_array($results, MYSQLI_BOTH)) {
            ...
            $cotizaciones .= '<tr><td>'.$row["Accion"].'</td><td>'.$row["Precio"].'</td></tr>';
               
        }
        
        $cotizaciones .= '</table> </div> </div>';
        // it releases the associated results
        mysqli_free_result($results);
        disconnect($conexion);
        return $cotizaciones;
